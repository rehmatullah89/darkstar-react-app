  ...
  useLayoutEffect(() => {
    ...
    if(resetButton){
      ...
      resetButton.addEventListener("click", (event) => {
        ...
        try {
          const filter_products = document.getElementById('filter_products') as HTMLSelectElement;
...
if(productList){
  ...
  landingShortcodes.forEach((shortcode: any) => {
    ...
    if (shortcode.parentNode) {
      ...
      colClass = `${shortcode.closest(".wpb_column").className
        } landing-product`;
    }
  ...
  Array.prototype.forEach.call(productList?.children, (child, index) => {
    ...
    const addremClass = () => {
      ...
      const filterProductsVal = (document.querySelector('#filter_products') as HTMLSelectElement).value;
      if (priceSortVal !== 'default' || filterProductsVal !== 'all') {

        (document.querySelector('#wrapper') as HTMLElement).classList.add('grid-changed');
        (document.querySelector('#product-list') as HTMLElement).classList.add('grid-changed');
      } else {
        (document.querySelector('#wrapper') as HTMLElement).classList.remove('grid-changed');
        (document.querySelector('#product-list') as HTMLElement).classList.remove('grid-changed');
      }
    }
    const show_hide_div_by_tag = (tag_val: string) => {
      if (tag_val === 'all') {
        document.querySelectorAll('.landing-product').forEach((element: Element) => {
    ...
    const sortAsc = (attr) => {
      ...
      if (product_list) {
        ...
        let landingProducts: any = document.querySelectorAll('.landing-product');
        product_list.innerHTML = '';
        const landingProductsArray = Array.from(landingProducts);
        landingProductsArray.sort(function (a, b) {
          return Number((a as HTMLElement).dataset[attr]) - Number((b as HTMLElement).dataset[attr]);
        });
    ...
    const sortDesc = (attr) => {
      ...
      if (product_list) {
        ...
        const landingProductsArray = Array.from(landingProducts);
        landingProductsArray.sort(function (a, b) {
          return Number((b as HTMLElement).dataset[attr]) - Number((a as HTMLElement).dataset[attr]);
        });
        landingProductsArray.forEach(function (product: any) {
          product_list.appendChild(product);
        });
    ...
    const filterProducts = document.getElementById('filter_products') as HTMLSelectElement | null;
    if (filterProducts) {
    ...
    if(addToCartButtons){
      addToCartButtons.forEach((add_to_cart)=>{
        ...
        if(add_to_cart){
          ...
          let  vipSalePrice = event?.target?.getAttribute("data-vip_sale_price");
          ...
          const radioButtons = productSelectionBox.querySelectorAll('input[type="radio"]');
          ...
          if (productId) {
            if(vipSalePrice){
              let vip_sale_attr = { //to pass customized vip sale price
                vip_user: 'yes',
    ...
    if(selectQuantity){
      selectQuantity.forEach((item:any)=>{
        item.addEventListener('click',(event:any)=>{
          document.querySelectorAll('.openPackage-quantity').forEach((element) => {
            element.classList.remove('openPackage-quantity');
          });
          const quantityWrapper =  event?.target.closest('.product-selection-description-parent-inner')
          if(quantityWrapper){
    ...
    if(selectPackage){
      selectPackage.forEach((item:any)=>{
         item.addEventListener('click',(event:any)=>{
          ...
          const packageWrapper =  event?.target.closest('.selectPackageWrapper ')
          if(packageWrapper){
    ...
    document.addEventListener('click', (event: MouseEvent) => {
      if (!(event.target as HTMLElement).closest('.product-selection-price-wrap')) {
    ...
    const crossIconQuantity = document.querySelectorAll('.packageQuantityBox  a.iconCloseBox')
    if(crossIconQuantity){
      crossIconQuantity.forEach((item:any)=>{
        item.addEventListener('click',(event)=>{
          (event.target as HTMLElement).closest('.product-selection-description-parent-inner')?.classList.remove('openPackage-quantity');
        })
      })
    }
    const crossIconPackage = document.querySelectorAll('.packageheader  a.iconCloseBox')
    if(crossIconPackage){
      crossIconPackage.forEach((item:any)=>{
        item.addEventListener('click',(event)=>{
          (event.target as HTMLElement).closest('.selectPackageWrapper')?.classList.remove('openPackage');
        })
      })
    }
    ...
    const selectAPackageElements = document.querySelectorAll('.select-a-package');
    selectAPackageElements.forEach((element) => {
      element.addEventListener('click', handleSelectPackage);
    });

    const selectors_inputs =  document.querySelectorAll('input[name="fav_language"]')
  ...
  const handleOptionChanges = (event:any)=>{
    ...
    if(sale_price){
      ...
      const add_to_cart_button = parentPackageBox.querySelector('.add_to_cart_button')
...
if(sale_price){
  ...
  const handleSelectPackage = (event: any) => {
    ...
    const packageSelectedDataElements = event.target.closest('.product-selection-box')?.querySelectorAll('.pacakge_selected_data');
    if (packageSelectedDataElements) {
      const lastElement = packageSelectedDataElements[packageSelectedDataElements.length - 1];
      if (lastElement) {
  ...
  const updatePrice = (box: Element) => {
    const price = (box as HTMLElement)?.closest('.product-selection-box')?.querySelector('.selectquantitybtn')?.getAttribute('data-price') || null;
    if(!price) return;
    const quantityInput = box.querySelector('.quantity') as HTMLInputElement | null;;
    if (!quantityInput) return;
  ...
  const addToCart = async (product_id: number | string,quantity:number=1,custom_attr?:any) => {
    try {
...
#product-list.productLandingPageContainer .medium-12.thb-dark-column.small-12.landing-product .product-selection-description-parent-inner {
  ...
  #product-list:not(.grid-changed) .medium-12.thb-dark-column.small-12.landing-product >.vc_column-inner{width:100vw;position:relative;left:calc(-50vw + 50%);padding-top:40px;padding-bottom:40px}
  ...
  #product-list .medium-12 .featureTag{background:#f8a18a}
  #product-list.grid-changed .medium-9 .product-selection-image-wrap{min-height:254px;max-width:408px}
  .teethWhieteingSystemWrapper.grid-changed .landing-product .product-selection-description-parent{background:#f1f1f8}
  #product-list.grid-changed .landing-product .product-selection-description-parent{padding:15px;min-height:86px}
  #product-list.grid-changed .landing-product .product-selection-price-wrap{padding-top:0}
  ...
  .grid-changed .wpb_column{max-width:50%!important;flex:0 0 50%}
  ...
  @media screen and (min-width: 768px) {
      #product-list.productLandingPageContainer .thb-dark-column.medium-12.landing-product{margin-bottom:25px}
...
#product-list .medium-6.landing-product .product-selection-description-parent-inner   .normalyAmount.italic:not(.forOnlyLayoutSixDisplay)
  ...
  #newsalePage #product-list.productLandingPageContainer .medium-12.thb-dark-column.small-12.landing-product>.vc_column-inner{background-color:#ebedee!important}
  ...
  #newsalePage #product-list:not(.grid-changed) .medium-12.thb-dark-column.small-12.landing-product .product-selection-description-parent{justify-content:center}
  #newsalePage #product-list.productLandingPageContainer .medium-12 .product-selection-description b{color: #565759;
    font-size: 28px;
    font-weight: 700;}
  #newsalePage .teethWhieteingSystemWrapper.productLandingPageContainer .landing-product:not(.medium-12) .product-selection-description-parent,#newsalePage .packageQuantityBox,.selectPackageBox{background:#f1f1f8}
  ...
  #newsalePage #product-list.grid-changed .medium-12 .product-selection-image-wrap img{max-width:100%}
  ...
  #newsalePage #product-list.grid-changed .medium-12 .product-selection-description .productDescriptionDiv{max-height:44px;overflow:hidden;min-height:44px;display:flex;align-items:flex-end}
  ...
  #newsalePage #product-list .columns:not(.medium-12) span.product-selection-price-text del:before{left:-1px;height:2px;background:#f8a18a}
...
.active-recomendation-tab  #product-list .medium-6 .product-selection-price-text-top{
  ...
  #newsalePage #product-list.grid-changed .medium-12.thb-dark-column.small-12.landing-product .product-selection-description-parent{max-width:inherit;width:auto;min-height:130px;max-height:130px}
  ...
  section.shopLanderPageHader .pageheaderBotm select{padding:0 30px 0 10px;min-width:186px;font-family:'Montserrat'}
  }
  ...
  #product-list.productLandingPageContainer .medium-12 .product-selection-price-text-top{margin-top:6px;margin-bottom:6px}
  ...
  #newsalePage div#product-list{margin-top:24px}
  section.shopLanderPageHader .pageheaderBotm select{font-size:12px}
...
,.productLandingPageContainer span.woocommerce-Price-amount.amount,  #newsalePage #product-list .product-selection-price-text
  ...
  @media only screen and (max-width: 1000px) and  (orientation: landscape){
        ...
        #product-list:not(.grid-changed) .medium-12.thb-dark-column.small-12.landing-product .product-selection-description-parent-inner .btn-primary-blue{
          max-width: 170px;
          min-width: 170px;
        }
      }
      ...
      @media only screen and (max-width: 767px) {
        ...
        .medium-6,.medium-3,.grid-changed .wpb_column,#product-list.productLandingPageContainer .medium-12.thb-dark-column.small-12.landing-product .product-selection-description-parent, #product-list .product-selection-image-wrap {
          -webkit-box-flex: 0;
          -ms-flex: 0 0 50%;
